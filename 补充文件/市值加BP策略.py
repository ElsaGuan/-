'''
市值轮动策略：
每一个月根据新的市值排名，选取前10分位的小市值股票。
在小市值股票中选取高账面价值比率的股票，将持仓调整为新的组合股票
'''
...
# 输出：股票list   
def get_stocks(stock_list, percent,context):
    ...
    stocks = stocks[:int(len(df_CMC)*percent)]
    return stocks

def get_stock_bp(stock_list, context):
    # 获得PB比率
    df_pb = get_fundamentals(query(valuation.code,valuation.pb_ratio).filter(valuation.code.in_(stock_list)))
    # 计算BP比率
    df_pb['BP'] = df_pb['pb_ratio'].apply(lambda x: 1/x)
    df_pb = df_pb.dropna()
    df_pb.set_index('code', drop = True, inplace = True)
    # 根据BP比率，由高到低排序
    df_pb.sort('BP', ascending  = False, inplace = True)
    stocks = list(df_pb.index)
    # 选取排名前十的股票
    stocks = stocks[:10]
    return stocks
...
def daily(context):
    ...
    if g.days % g.period == 0:
        ...
        # 根据市值选取小市值股票
        stock_list = get_stocks(feasible_stocks, g.percent, context)
        
        # 根据BP值选取股票，选取BP值大的股票
        holding_list = get_stock_bp(stock_list, context)
